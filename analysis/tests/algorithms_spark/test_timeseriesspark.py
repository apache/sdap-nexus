...
from webservice.algorithms_spark.TimeSeriesSpark import TimeSeriesSparkHandlerImpl, TimeSeriesResults
...
def test_tile():
    yield Tile(
        ...
        min_time='2020-01-28T00:00:00',
        max_time='2020-01-28T00:00:00',
        section_spec='2020-01-28T00:00:00',
        meta_data={},
        is_multi=True
    )
...
def test_timeseries_args():
    ...
    yield dict(
        tile_ids=tile_ids,
        ds_b=MockSparkParam(ds_name),
        start_b=MockSparkParam(start),
        end_b=MockSparkParam(end),
        seasonal_b=MockSparkParam(seasonal),
        low_pass_b=MockSparkParam(low_pass),
...
def test_calc(test_timeseries_args):
    ...
    with mock.patch('webservice.algorithms_spark.TimeSeriesSpark.TimeSeriesResults') as mock_rs, \
            mock.patch(
                'webservice.algorithms_spark.TimeSeriesSpark.spark_driver') as mock_driver:
        ...
        matchup_driver_args = mock_driver.call_args_list[0].args
        matchup_driver_kwargs = mock_driver.call_args_list[0].kwargs
        print(matchup_driver_args)
        exit()
