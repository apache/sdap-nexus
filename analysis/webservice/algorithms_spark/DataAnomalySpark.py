...
@nexus_handler
class DataAnomalySparkHandlerImpl(SparkHandler):
    name = "Data Anomaly Spark"
    path = "/dataAnomalySpark"
    description = "TODO Computes a Latitude/Longitude Time Average plot given an arbitrary geographical area and time range"
    params = {
        "ds": {
            "name": "Dataset",
            "type": "String",
    ...
    def calc(self, compute_options, **args):
        ...
        list_without_tuple = [x[1] for x in avg_tiles]
        list_of_means = []
        for two_dim in list_without_tuple:
            for one_dim in two_dim:
                for x in one_dim:
                    if x['avg'] != fill:
                        list_of_means.append(x['avg'])

        mean_of_means = np.mean(list_of_means)

        # Remove the mean of means from each mean value
        updated_avg_tiles = \
            sum_count.map(lambda (bounds, (sum_tile, cnt_tile)):
                          (bounds, [[{'avg': (sum_tile[y, x] / cnt_tile[y, x]) - mean_of_means
                          if (cnt_tile[y, x] > 0)
                          else fill,
                                      'cnt': cnt_tile[y, x]}
                                     for x in
        ...
        #     tile.update({'avg': tile['avg'] - mean_of_means} if tile['avg'] != fill else {'avg': tile['avg']})
